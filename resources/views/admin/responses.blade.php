        ...
        @foreach (($form['questions'] ?? []) as $index => $q)
          <div class="mb-6">
            ...
            @switch($q['type'])
              ...
              @case('short')
              @case('name')
                <input type="text" name="answers[{{ $index }}]" class="w-full border px-3 py-2 rounded"
                  @if(!empty($q['required'])) required @endif value="{{ old('answers.'.$index) }}">
                @break
              ...
              @case('email')
                <input type="email" name="answers[{{ $index }}]" class="w-full border px-3 py-2 rounded"
                  @if(!empty($q['required'])) required @endif value="{{ old('answers.'.$index) }}"
                  pattern="^[\w\-.]+@[\w\-]+\.[a-z]{2,}$">
                @break

              @case('mobile')
                <input type="tel" name="answers[{{ $index }}]" pattern="[0-9]{10,15}" maxlength="15"
                  inputmode="numeric"
                  class="w-full border px-3 py-2 rounded"
              ...
              @case('age')
                <input type="number" name="answers[{{ $index }}]" min="1" max="120"
                  class="w-full border px-3 py-2 rounded"
                  @if(!empty($q['required'])) required @endif
                  value="{{ old('answers.'.$index) }}">
                @break
              ...
              @case('file')
                <input type="file" name="answers[{{ $index }}]" class="w-full border px-3 py-2 rounded"
                  @if(!empty($q['required'])) required @endif>
                @break
              ...
              @case('location')
                <input type="text" name="answers[{{ $index }}]" id="location_{{ $index }}"
                  class="w-full border px-3 py-2 rounded"
                  readonly @if(!empty($q['required'])) required @endif value="{{ old('answers.'.$index) }}">
                <button type="button"
              ...
              @case('multiple')
                @foreach($q['options'] ?? [] as $opt)
                  <label class="block">
                    <input type="checkbox" name="answers[{{ $index }}][]" value="{{ $opt }}"
              ...
              @case('date')
                <input type="date" name="answers[{{ $index }}]"
                  class="w-full border px-3 py-2 rounded"
                  @if(!empty($q['required'])) required @endif
                  value="{{ old('answers.'.$index) }}">
                @break
